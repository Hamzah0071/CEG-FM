document.addEventListener('DOMContentLoaded', () => {
    ...
    window.toggleAddForm = function () {
        ...
        document.querySelector('#matierInput').value = '';
    ...
    confirmAddBtn.addEventListener('click', (e) => {
        ...
        const matier = document.querySelector('#matierInput').value.trim();
        const date = document.querySelector('#dateInput').value.trim();

        if (!name || !className || !matier || !date) {
    ...
    tbody.addEventListener('click', (e) => {
        ...
        // ✅ Valider modification
            ...
            const [nameInput, classInput, matierInput, dateInput] = inputs;

            if (!nameInput.value.trim() || !classInput.value.trim() || !matierInput.value.trim() || !dateInput.value.trim()) {
                alert('Veuillez remplir tous les champs.');
                return;
            }
            ...
            const cells = row.querySelectorAll('td:not(:last-child):not(:first-child)');
            cells[0].textContent = nameInput.value.trim();
            cells[1].textContent = classInput.value.trim();
            cells[2].textContent = matierInput.value.trim();
